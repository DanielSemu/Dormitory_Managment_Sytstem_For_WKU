...
def PlaceStudent(request):
    ...
    role_id=role.id
    student=UserAccount.objects.filter(Role_id=role_id)
    ...
    for i in student:
        Id.append(i.User_id)
    for i in Id:
        result=User.objects.get(id=i)
        usc=UserAccount.objects.get(User_id=result.id)
        if Placement.objects.filter(Stud_id_id=usc.id).exists():
            pass
        else:
    ...
    if request.method=="POST":
        ...
        if searched =="":
            ...
            for i in Id:
                result=User.objects.get(id=i)
                if result.collage == searched or result.Department == searched or result.Year_of_Student==searched:
                    usc=UserAccount.objects.get(User_id=result.id)
                    if Placement.objects.filter(Stud_id_id=usc.id).exists():
                        pass
                    else:
                    ...
                    if str(student[9]) ==str(block.Block_type):
                        if str(dorm1['Floor'])=='Floor-1':
                            ...
                            if int(cnt)<int(dorm1['Capacity']):
                                ...
                                if Placement.objects.filter(Stud_id=usracc.id).exists():
                                 pass
                                else:
                                    ...
                                    place=Placement(Stud_id=userr,Block=block,Room=room)
                                    place.save()
                                    count=count+1
                            else:
                                break
            ...
            if result != "":
                for i in result:
                    if UserAccount.objects.filter(User_id=i.id).exists():
                        ...
                        if Placement.objects.filter(Stud_id_id=acc.id):
                            pl=Placement.objects.get(Stud_id_id=acc.id)
                            user_id.append(i)
            ...
            if Block.objects.filter(Block_name=searched).exists():
                ...
                if Placement.objects.filter(Block_id=block.id).exists():
                    ...
                    for i in pl:  
                        ...
                        usr=User.objects.get(id=uracc.User_id)
                        user_id.append(usr)
                lst=[{'item1':t[0],'item2':t[1],'item3':t[2],'item4':t[3]} for t in zip(user_id,Block_name1,Dorm_name1,result1)] 
                context={'List':lst}
                setting=settings()
...
def updateStudent(request,pk):
    if 'username' in request.session:
        ...
        user=User.objects.get(id=user_acc.User_id)
        ...
        if request.method=='POST':
            ...
            if result != "":
                for i in result:
                    if UserAccount.objects.filter(User_id=i.id).exists():
                        acc=UserAccount.objects.get(User_id=i.id)
                        if Placement.objects.filter(Stud_id_id=acc.id).exists():
                            pl=Placement.objects.get(Stud_id_id=acc.id)
                            user_id.append(i)
                            blk=Block.objects.get(id=pl.Block_id)
                            drm=Dorm.objects.get(id=pl.Room_id)
    ...
    if request.method =='POST':
        ...
        for i in StudentId:
            ...
            if Material.objects.filter(student_id=acc.id):
                ...
                plc=Placement.objects.get(Stud_id_id=acc.id)
                plc.delete()
                count+=1
        # if count1 !=0:
        if count==0:
