...
def request_material(request):
    if 'username' in request.session:
        ...
        if Borrow_Request.objects.filter(student_id=user1).exists():
            ...
            if Placement.objects.filter(Stud_id_id=user1).exists():
    ...
    if 'username' in request.session:
        ...
        if Placement.objects.filter(Stud_id_id=acc.id):
            if request.method=='POST':
                ...
                today=date.today()
                if Exit_Permission_Requst.objects.filter(student_id=acc.id).exists() or Exit_Permission.objects.filter(date=today).exists():
...
def reportProblem(request):
 if 'username' in request.session:
    ...
    if request.method=='POST':
        ...
        username = request.session['username']
        acc=UserAccount.objects.get(username=username)
        if Placement.objects.filter(Stud_id=acc.id).exists():
