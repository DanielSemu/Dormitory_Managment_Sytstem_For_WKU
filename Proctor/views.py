...
def borrow_material(request):
    if 'username' in request.session:
        ...
        if ProctorAssignment.objects.filter(user_id=key).exists():
            ...
            for i in accs:
                usr=UserAccount.objects.get(id=i)
                studs_id.append(usr.User_id)
            for i in studs_id:
...
def report_view(request):
    ...
    if 'username' in request.session:  
        ...
        if ReportProblem.objects.filter(is_solve='Un-Approved').exists():
          ...
          for i in report:
              if Placement.objects.filter(Stud_id=i.student).exists():
                  stud=Placement.objects.get(Stud_id=i.student)
